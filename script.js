/* ============================================================
   ...
   Isolated 3D scenes. This is the one part of the page that
   depends on WebGL + ES module imports. Both the big hero ball
   and the small Engineering-section ball are built here, sharing
   the exact same texture and rotation speed, so a WebGL failure
   never breaks the rest of the page (that all lives in interactions.js).
   ============================================================ */
try {
  ...
  // Build the basketball textures ONCE and share them between the big
  // hero ball and the small Engineering-section ball.
  const { colorTex, bumpTex } = buildBasketballTextures(THREE);

  initHeroScene(THREE, EffectComposer, RenderPass, UnrealBloomPass, colorTex, bumpTex);
  initEngineeringBall(THREE, colorTex, bumpTex);
} catch (err) {
  console.warn("Techletics: 3D scenes unavailable, showing CSS fallback.", err);
}
...
function initHeroScene(THREE, EffectComposer, RenderPass, UnrealBloomPass, colorTex, bumpTex) {
...
function initEngineeringBall(THREE, colorTex, bumpTex) {
  ...
  scene.add(new THREE.AmbientLight(0x1e2a3a, 0.9));

  const key = new THREE.DirectionalLight(0x8fc6ff, 2.2);
  key.position.set(-4, 4, 5);
  scene.add(key);

  const rim = new THREE.PointLight(0xff6a00, 40, 15, 2);
  rim.position.set(3, -1.5, -2);
  scene.add(rim);

  const ballGeo = new THREE.SphereGeometry(1.7, 96, 96);
  const ballMat = new THREE.MeshPhysicalMaterial({
    map: colorTex,
    bumpMap: bumpTex,
    bumpScale: 0.05,

...
(function () {
  ...
  if (hasGSAP) {
    ...
    // Single shared trigger + stagger for all 4 steps (instead of 4 separate
    // ScrollTrigger instances pointed at the identical trigger/start, which
    // could fire inconsistently). No opacity kill either — worst case the
    // steps are simply static, never invisible.
    var howSteps = gsap.utils.toArray(".how-step");
    if (howSteps.length) {
      gsap.from(howSteps, {
        y: 50, duration: 0.9, ease: "power3.out", stagger: 0.12,
        scrollTrigger: { trigger: ".how-track", start: "top 90%", once: true }
      });
    }
